...
Application::Application(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    // check update locks
    {
        ...
        if (update_lock.exists()) {
            ...
            auto infoMsg = tr("This installation has a update lock file present at: %1\n"
                              ...
                              "proceeding.\n"
                              "Check the Jarton Client updater log at: \n"
                              "%7\n"
    ...
    // renders with no style overlay (black background) and AnnouncementBar / stats /
    // changelog bindings resolve to "undefined" — the dead, empty home screen.
    initJartonServices();
...
void Application::setupWizardFinished(int status)
{
    ...
    // the manifest/wallpaper/first-launch provisioning services stay dead until
    // the user's second launch.
    initJartonServices();
    performMainStartupAction();
}
...
void Application::initJartonServices()
{
    ...
    registerService("JartonProvisionService", m_jartonProvision);
    ...
    // instance list is populated, in which case the first checkAll() finds no instance.
    if (auto* list = instances()) {
        connect(list, &InstanceList::dataChanged, m_jartonDefaultInstance,
                [this](const QModelIndex&, const QModelIndex&, const QList<int>&) {
                    m_jartonDefaultInstance->refresh();
                    m_jartonUpdate->checkAll();
                });
    }
    ...
    // First-launch provisioning: download the canonical instance zip from the CDN
    // and import it as "Jarton". Only fires when the instance is genuinely Missing,
    // so a user's existing/edited Jarton instance is never overwritten. Runs headless
    // (no ProgressDialog) — the home tab reflects the Provisioning state via QML.
    ...
    // pack as a fresh "Jarton <ver>" instance. Distinct name keeps it clear of the
    // canonical-instance and update-prompt flows, both of which key on exactly "Jarton".
    ...
    // user explicitly said yes and would otherwise wonder where the update went.
    connect(m_jartonUpdate, &Jarton::JartonUpdateService::instanceUpdateRequested, this,
            [this](const QString& instanceId, const QString& packUrl, const QString& mcVersion, const QString& packVersion) {
                BaseInstance* inst = instances()->getInstanceById(instanceId);
                if (inst == nullptr || inst->isRunning()) {
                    return;
                }
                qInfo() << "[jarton.update] updating" << inst->name() << "to pack" << packVersion;
                auto* task = new Jarton::JartonPackUpdateTask(inst->instanceRoot(), inst->gameRoot(), packUrl,
                ...
                connect(task, &Task::failed, this, [name](const QString& reason) {
                    ...
                    QMessageBox::warning(nullptr, tr("Jarton update failed"),
                                         tr("%1 couldn't be updated:\n%2\n\nThe instance was left as it was.").arg(name, reason));
                });
...
void Application::importJartonPack(const QString& packUrl,
                                   ...
                                   std::function<void()> onFinished)
{
    ...
    importTask->setName(instanceName);
    // Keep the pack's own iconKey + bundled icon. Without this the default empty icon
    // takes the "user picked an icon" branch in processMultiMC and overwrites iconKey
    // with "", falling back to the Minecraft block.
    importTask->setIcon(QStringLiteral("default"));
    // wrapInstanceTask stages + commits into the InstanceList; parent the wrapper to the
    ...
    connect(staged, &Task::succeeded, this, [this, instanceName, mcVersion, packVersion] {
        ...
        for (int i = 0; i < instances()->count(); ++i) {
            BaseInstance* inst = instances()->at(i);
            if (inst == nullptr || inst->name() != instanceName) {
                continue;
            }
    ...
    connect(staged, &Task::failed, this,
            [instanceName](QString reason) { qWarning() << "[jarton.instance] pack import failed:" << instanceName << reason; });
    connect(staged, &Task::finished, this, [staged, onFinished]() {
        if (onFinished) {
            onFinished();
        }
        staged->deleteLater();
