...
MacSparkleUpdater::MacSparkleUpdater() {
    ...
    // Scope Sparkle's autoreleased setup temporaries to this block. A pool held
    // as a member and drained in the destructor pops out of LIFO order at app
    // shutdown and aborts (badPop on macOS); the objects below are alloc/init
    // (owned) and retained by priv, so they outlive the pool.
    @autoreleasepool {
        ...
        // KVO callback emits a Qt signal when `canCheckForUpdates` changes, so the UI can respond.
