...
void MinecraftInstance::jartonInjectUiMod(const QString& sourceJar)
{
    if (sourceJar.isEmpty() || !QFile::exists(sourceJar)) {
        return;  // no bundled jar for this loader (e.g. NeoForge/Forge before the port ships)
    }
    const QString mods = modsRoot();
    if (!FS::ensureFolderPathExists(mods)) {
        return;
    }
    const QString dest = FS::PathCombine(mods, "JartonUI.jar");

    QDir dir(mods);
    const QStringList present =
        dir.entryList({ QStringLiteral("JartonUI*.jar"), QStringLiteral("JartonUI*.jar.disabled") }, QDir::Files);

    // If the canonical jar is already the only variant present and byte-identical to the bundle, do
    // nothing. Rewriting on every launch churns the mods folder and triggers a QFileSystemWatcher
    // storm that re-scans the model repeatedly — the root cause behind the Edit→Mods crashes.
    const bool onlyCanonical = present.size() == 1 && present.first() == QStringLiteral("JartonUI.jar");
    if (onlyCanonical && jarsIdentical(sourceJar, dest)) {
        return;
    }

    // Drop any prior copy (incl. read-only versioned names shipped in a pack) so a refreshed
    // bundle replaces it cleanly instead of stacking duplicates or blocking the copy below.
    for (const QString& name : present) {
        const QString path = FS::PathCombine(mods, name);
        QFile::setPermissions(path, QFileDevice::ReadOwner | QFileDevice::WriteOwner);  // pack jars ship read-only
        QFile::remove(path);
    }
    QFile::remove(dest);  // QFile::copy won't overwrite; make sure the slot is free
    if (QFile::copy(sourceJar, dest)) {
        // Resource copies come out read-only; make it writable so the next launch can replace it.
        QFile::setPermissions(dest, QFileDevice::ReadOwner | QFileDevice::WriteOwner | QFileDevice::ReadGroup | QFileDevice::ReadOther);
    }
}

bool MinecraftInstance::jarsIdentical(const QString& a, const QString& b)
{
    QFile fa(a);
    QFile fb(b);
    ...
    QCryptographicHash ha(QCryptographicHash::Sha1);
    QCryptographicHash hb(QCryptographicHash::Sha1);
    if (!ha.addData(&fa) || !hb.addData(&fb)) {
        return false;
    }
    return ha.result() == hb.result();
}

void MinecraftInstance::jartonSeedGuiScale()
{
    // Minecraft's default GUI scale is "auto" (guiScale:0), which is enormous on Retina.
    // Force a usable scale of 2 when the instance has no scale set OR is still on auto, but
    // leave any deliberate non-zero value the player picked alone.
    const QString optionsPath = FS::PathCombine(gameRoot(), "options.txt");
    QFile f(optionsPath);

    if (!f.exists()) {
        if (!FS::ensureFolderPathExists(gameRoot()) || !f.open(QIODevice::WriteOnly | QIODevice::Text))
            return;
        f.write("guiScale:2\n");
        f.close();
        return;
    ...
    for (QString& line : lines) {
        if (line.startsWith(QStringLiteral("guiScale:"))) {
            found = true;
            if (line.trimmed() == QStringLiteral("guiScale:0")) {  // "auto" — the unusable one
                line = QStringLiteral("guiScale:2");
                changed = true;
            }
            break;
        }
    ...
    if (!found) {
        if (!lines.isEmpty() && lines.last().isEmpty())
            lines.last() = QStringLiteral("guiScale:2");  // reuse the trailing-newline slot
        else
            lines.append(QStringLiteral("guiScale:2"));
        changed = true;
    }
    if (!changed)
        return;
...
QString MinecraftInstance::createLaunchScript(AuthSessionPtr session, MinecraftTarget::Ptr targetToJoin)
{
    ...
    // instance info
    {
        ...
        // Intentionally do NOT pass instanceIconPath: the launcher list shows the
        // instance's icon, but the running game keeps Minecraft's own window/taskbar
        // icon rather than inheriting the Jarton instance icon.
    }
