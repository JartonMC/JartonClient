...
class MinecraftInstance : public BaseInstance {
   ...
   protected:
    ...
    // Copy the bundled Jarton UI mod into this instance's mods folder for a given loader's jar.
    void jartonInjectUiMod(const QString& sourceJar);
    // True when both jars exist and are byte-identical (size then SHA-1). Used to skip a redundant
    // rewrite of the injected UI jar so we don't churn the mods folder on every launch.
    static bool jarsIdentical(const QString& a, const QString& b);
    // Seed a sane default GUI scale (2) the first time an instance launches, so it doesn't
    // come up at the giant "auto" scale on high-DPI displays. Never overrides a set value.
    void jartonSeedGuiScale();
