...
SwiftyApi::SwiftyApi(SwiftyClient* client, QObject* parent)
    : QObject(parent), m_client(client), m_nam(client ? client->network() : nullptr)
{
}

int SwiftyApi::send(const QString& method, const QString& path, const QString& jsonBody)
{
    const int id = m_nextId++;
    if (!m_client || !m_nam || m_client->token().isEmpty()) {
        return id;
    }

    QNetworkRequest req{ QUrl(m_client->baseUrl() + path) };
    req.setRawHeader("Authorization", "Bearer " + m_client->token().toUtf8());
