...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    {  // "Create Jarton Instance" — the only way to create an instance; the generic Add Instance is gone.
        m_actionCreateJartonInstance = new QAction(tr("Create &Jarton Instance"), this);
        m_actionCreateJartonInstance->setToolTip(tr("Set up a fresh instance with the Jarton mods for a chosen version"));
        m_actionCreateJartonInstance->setIcon(QIcon::fromTheme("new"));
        connect(m_actionCreateJartonInstance, &QAction::triggered, this,
                &MainWindow::on_actionCreateJartonInstance_triggered);
        QAction* toolBarHead = ui->mainToolBar->actions().value(0);
        ui->mainToolBar->insertAction(toolBarHead, m_actionCreateJartonInstance);
        ui->mainToolBar->insertSeparator(toolBarHead);
        ui->fileMenu->insertAction(ui->fileMenu->actions().value(0), m_actionCreateJartonInstance);
    }
    ...
    // Global shortcuts
    {
        ...
        m_actionCreateJartonInstance->setShortcut(QKeySequence::New);
...
void MainWindow::showInstanceContextMenu(const QPoint& pos)
{
    ...
    if (onInstance) {
        ...
        QAction* actionCreateInstance = new QAction(tr("&Create Jarton instance"), this);
        actionCreateInstance->setToolTip(m_actionCreateJartonInstance->toolTip());

        connect(actionCreateInstance, &QAction::triggered, this, &MainWindow::on_actionCreateJartonInstance_triggered);
...
void MainWindow::on_actionCreateJartonInstance_triggered()
{
    createJartonInstance();
}

void MainWindow::createJartonInstance()
{
    JartonInstanceDialog dlg(this);
    if (dlg.exec() != QDialog::Accepted) {
        return;
    }
    const QString version = dlg.selectedVersion();
    if (version.isEmpty()) {
        return;
    }
...
void MainWindow::repositionFloatingOverlays()
{
    if (m_centralBg != nullptr && m_changelogPanel != nullptr) {
        ...
        // The grid yields the panel's column while it's open: inset the top row
        // so tiles wrap at the panel's left edge instead of flowing under it.
        if (m_centralTopRow != nullptr) {
            const int inset = m_changelogPanel->isVisible() ? m_centralBg->width() - panelX + 18 : 0;
            m_centralTopRow->setContentsMargins(0, 0, inset, 0);
        }
