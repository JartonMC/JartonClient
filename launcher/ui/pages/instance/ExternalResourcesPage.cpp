...
void ExternalResourcesPage::removeItem()
{
    ...
    for (auto& i : selection.indexes()) {
        if (i.column() == 0 && m_model->validateIndex(i)) {
            ...
            const auto& info = m_model->at(i.row()).fileinfo();
            if (info.isDir()) {
                folder = true;
...
void ExternalResourcesPage::updateFrame(const QModelIndex& current, [[maybe_unused]] const QModelIndex& previous)
{
    ...
    // mapToSource can hand back an invalid index (row -1) when the source model was reset out from under
    // the selection — e.g. the file watcher re-scanning the mods folder after the injected jar is refreshed.
    // Dereferencing at(-1) on that stale row is the random-click crash.
    if (!m_model->validateIndex(sourceCurrent)) {
        ui->frame->clear();
        return;
    }
    Resource const& resource = m_model->at(sourceCurrent.row());
