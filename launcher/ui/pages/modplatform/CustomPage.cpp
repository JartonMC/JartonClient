...
void CustomPage::updateLoaderHint()
{
    static const QString general =
        tr("Hand-built instances won't include the mods set up for the intended Jarton experience.");
    static const QString incompatible =
        tr("Jarton UI won't run on this instance.");

    const bool noVersion = !m_selectedVersion;
    const bool compatibleLoader =
        ui->fabricFilter->isChecked() || ui->quiltFilter->isChecked() ||
        ui->neoForgeFilter->isChecked() || ui->forgeFilter->isChecked();
    // Fabric/Quilt need >= 1.14; below that no compatible loader exists for the version.
    const bool loaderAvailable =
        !noVersion &&
        (!(ui->fabricFilter->isChecked() || ui->quiltFilter->isChecked()) ||
         Version(m_selectedVersion->descriptor()) >= Version("1.14"));

    QString text = general;
    if (!compatibleLoader || !loaderAvailable) {
        text += "\n" + incompatible;
    }
    ui->loaderHint->setText(text);
    ui->loaderHint->setVisible(true);
}
